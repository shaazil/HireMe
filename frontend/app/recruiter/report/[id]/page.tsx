...
} from "@/components/report/candidate-profile-panel";
import { useAuthStore } from "@/store/auth-store";
import api from "@/services/api";
import { toast } from "sonner";

interface EvaluationReport extends CandidateProfileData {
  id: string;
  session_id: string;
...
export default function RecruiterReportPage() {
  ...
  const updateApplicationStatus = async (status: string) => {
    const previousStatus = report?.application_status;
    ...
    try {
      ...
      if (previousStatus) {
        setReport((prev) =>
          prev ? { ...prev, application_status: previousStatus } : null
        );
      }
    }
  };
  ...
  if (isLoading) {
    return (
      <div className="flex flex-col h-[calc(100vh-120px)] items-center justify-center text-muted-foreground gap-3">
        ...
        <p className="text-[14px]">Loading evaluation report...</p>
      </div>
    );
  }
    ...
    return (
      <div className="flex flex-col h-[calc(100vh-120px)] items-center justify-center text-center">
        ...
        <p className="text-muted-foreground text-sm mb-6 max-w-sm">
          We couldn&apos;t find the evaluation report for this session.
        </p>
    ...
    <div className="max-w-6xl mx-auto pb-12">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-4">
          <CandidateProfilePanel profile={report} showResume />
        </div>
            ...
            <div>
              ...
              <p className="text-muted-foreground text-[14px]">
                ...
                <span className="capitalize font-medium text-foreground">
                  {report.application_status?.replace("_", " ")}
                </span>
              </p>
            </div>
            <div className="flex items-center gap-4 flex-wrap">
              {user?.role === "recruiter" &&
                report.application_status !== "hired" &&
                report.application_status !== "rejected" && (
          ...
          <div className="grid grid-cols-1 md:grid-cols-12 gap-6 mb-8">
            <div className="md:col-span-4 bg-card border border-border rounded-2xl p-8 flex flex-col items-center justify-center text-center shadow-sm">
              ...
              <p className="text-[12px] text-muted-foreground mt-4 max-w-[200px] leading-relaxed">
                Composite score from interview, coding, and communication.
              </p>
            </div>
